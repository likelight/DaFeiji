function sendAjax(type, channel) {
...
function showType (type) {
    if (type == 'free' || type == '10' || type == '49') {
        $('.page').hide();
        $('.win-page').show();
        $('#type-' + type).css({
            display: 'block'
        });
        window.hasShowType = type;
        if (type == '49') {
           // _mz_evt('1001264', '100031909');
        } else if (type == '10') {
           // _mz_evt('1001264', '100031911');
        } else {
           // _mz_evt('1001264', '100031913');
        }
    } else if (type == 'lose') {
        $('.page').hide();
        $('.lose-page').show();
       // _mz_evt('1001264', '100031915');

    }
}

function checkType (type, channel) {

    sendAjax(type, channel).done(function (data) {
...
(function (window, createjs) {
    ...
    var gameManager = {
        ...
        handleProgress: function (data) {
            $('.progress-bar').css({
                width: (data.loaded * 100).toFixed(0) + '%'
            });
            var gap = (427 * window.innerWidth / 649 - 114 * window.innerWidth / 649) / 100;
            $('.load-plane').css({
                left: 114 * window.innerWidth / 649 + gap * (data.loaded * 100).toFixed(0) + 'px'
            });
            document.querySelector('.percent').innerHTML = (data.loaded * 100).toFixed(0) + '%';
        },
        handleLoadComplete: function () {
            $('.page').hide();
            $('.mashangkaida-page').show();
            //_mz_evt('1001264', '100031905');
        ...
        addFood: function (enemy) {
            ...
            var foodName = foods[Math.floor(Math.random() * 4)];
            var foodResult = this.loader.getResult(foodName);


            var food = new createjs.Bitmap(foodResult);
            food.x = enemy.x + 10;
            food.y = enemy.y + 10;
            ...
            this.config.foods.push(foodName);
            this.config.foodsMap[foodName] += 1;
        },
        ...
        updateBG: function (event) {
            ...
            bgContainer.y += event.delta / 1000 * 30;
        },
        initPlay: function () {
            this.config.boss.leftHandHp = 20;
            this.config.boss.rightHandHp = 20;
            // game interface
        ...
        startGame: function () {
            ...
            this.plane_bullet = createjs.Sound.play('plane_bullet', {
                loop: -1,
                duration: 1000
        ...
        gameOver: function () {
            ...
            this.bg_music.stop();
            this.plane_bullet.stop();

            if (this.config.foods.length > 40
                && this.config.foodsMap.bacon > 8
                && this.config.foodsMap.chicken > 8
                && this.config.foodsMap.mushroom > 8
                && this.config.foodsMap.cheese > 8) {
                // alert('checkFree');
                checkType('free', 'h5');

            } else if (this.config.foods.length > 15) {
                 // alert('check49');
                checkType('49', 'h5');
            } else if (this.config.foods.length > 3) {
                // alert('check10');
                checkType('10', 'h5');
            } else {
                // alert('lose');
